...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function onScroll() {
        ...
        var offset = 120; // adjust for sticky header height
        var found = false;
        sections.forEach(function(section, i) {
            if (
                scrollPos + offset >= section.offsetTop &&
                scrollPos + offset < section.offsetTop + section.offsetHeight
            ) {
                navLinks.forEach(function(l) {
                    if (l.parentElement) l.parentElement.classList.remove("active");
                });
                if (navLinks[i] && navLinks[i].parentElement) {
                    navLinks[i].parentElement.classList.add("active");
                    found = true;
                }
            }
        });
        // Remove all if not found (top of page)
        if (!found) {
            navLinks.forEach(function(l) {
                if (l.parentElement) l.parentElement.classList.remove("active");
            });
